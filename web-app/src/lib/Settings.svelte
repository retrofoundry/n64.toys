...
<Panel title="settings" bodyClass="flex flex-col gap-2 p-3">
  <div class="flex items-center justify-between border border-edge bg-base px-3 py-2.5">
    ...
    <Select.Root
      type="single"
      bind:value={pg.settings.microcode}
      items={microcodes}
      onValueChange={() => {
    ...
    >
      <Select.Trigger aria-labelledby="microcode-label" class={triggerCls}>{pg.settings.microcode} ▾</Select.Trigger>
      <Select.Portal>
